...
const game = {
    canvas: null,
    ctx: null,
    level: 5,  // START AT LEVEL 5 (FINAL LEVEL!)
    score: 0,
    survivors: 70,  // Many survivors for final level
    totalPeople: 70,
    state: 'playing', // playing, freezing, transitioning, gameOver, ending
    ...
    createMockLevel1Frozen() {
        // Create multiple frozen layers (levels 1-4) since we're at level 5
        const screenHeight = config.canvas.height;
        
        // Create 4 frozen layers stacked
        const layerHeights = [
            screenHeight * 0.90, // Level 1 (bottom)
            screenHeight * 0.75, // Level 2
            screenHeight * 0.60, // Level 3
            screenHeight * 0.45  // Level 4 (top frozen layer)
        ];
        
        layerHeights.forEach((waterLevel, levelIndex) => {
            this.createFrozenLayer(levelIndex + 1, waterLevel);
        });
        
        // Set water to start just above the highest frozen layer
        this.water.y = layerHeights[layerHeights.length - 1] - 50;
        
        // Set target height even higher
        this.targetHeight = layerHeights[layerHeights.length - 1] - 250;
    },
    
    createFrozenLayer(level, waterLevel) {
        const level1WaterLevel = waterLevel;
